...
function updateUIFromSettings() {
	...
	if (keyframeSettings) {
		...
		document.getElementById("commonSaturationValue").textContent = commonSaturation + "%";
		...
		document.getElementById("commonLightnessValue").textContent = commonLightness + "%";
		...
		document.getElementById("commonThicknessValue").textContent = commonThickness.toFixed(2);
		...
		document.getElementById("commonAmpFactorValue").textContent = commonAmpFactor.toFixed(2);
		...
		document.getElementById("commonFreqFactorValue").textContent = commonFreqFactor.toFixed(2);
		...
		document.getElementById("commonDirectionOffsetValue").textContent = commonDirectionOffset + "°";
		...
		document.getElementById("phaseOffsetValue").textContent = phaseOffset.toFixed(2) + "°";
		...
		document.getElementById("commonOffsetValue").textContent = commonOffset.toFixed(2) + "°";
		
		// Vector offset controls (if applicable)
		if (appSettings.waveOffsetType === "vector") {
			document.getElementById("offsetAngle").value = keyframeSettings.offsetAngle || 0;
			document.getElementById("offsetAngleValue").textContent = (keyframeSettings.offsetAngle || 0) + "°";
...
function createGridControls(keyframeSettings) {
	...
	// Use the global gridCount from appSettings, not from the keyframe
	const gridCount = appSettings.gridCount;
	
	console.log(`Using gridCount=${gridCount}`);
	...
	// Update button states based on current active grids
...
function updateGridControlValues(keyframeSettings) {
	...
	for (let i = 0; i < gridCount; i++) {
		...
		const gridControl = document.querySelector(`.grid-control[data-index="${i}"]`);
		if (!gridControl) continue; // Skip if control not found
		
		// Update active checkbox
		const activeCheckbox = gridControl.querySelector(`.gridActive[data-index="${i}"]`);
		if (activeCheckbox) {
			activeCheckbox.checked = grid.isActive !== false;
			
			// Update inactive styling
			if (grid.isActive !== false) {
				gridControl.classList.remove('inactive-control');
				gridControl.querySelectorAll('input, button').forEach(element => {
					if (!element.classList.contains('gridActive')) {
						element.disabled = false;
					}
				});
			} else {
				gridControl.classList.add('inactive-control');
				gridControl.querySelectorAll('input, button').forEach(element => {
					if (!element.classList.contains('gridActive')) {
		...
		// Update input values
		const thicknessInput = gridControl.querySelector(`.lineThickness[data-index="${i}"]`);
		if (thicknessInput) thicknessInput.value = grid.thickness;
		
		const widthInput = gridControl.querySelector(`.cellWidth[data-index="${i}"]`);
		if (widthInput) widthInput.value = grid.width;
		
		const heightInput = gridControl.querySelector(`.cellHeight[data-index="${i}"]`);
		if (heightInput) heightInput.value = grid.height;
		
		const colorInput = gridControl.querySelector(`.color[data-index="${i}"]`);
		if (colorInput) colorInput.value = grid.color;
		
		// Update color button background
		const colorButton = gridControl.querySelector(`.custom-color-button[data-index="${i}"]`);
...
function updateWaveControlValues(keyframeSettings) {
	...
	for (let i = 0; i < waveCount; i++) {
		...
		// Update isActive state
		const waveControl = document.querySelector(`.wave-control[data-index="${i}"]`);
		if (!waveControl) continue; // Skip if control not found
		
		// Update active checkbox
		const activeCheckbox = waveControl.querySelector(`.waveActive[data-index="${i}"]`);
		if (activeCheckbox) {
			activeCheckbox.checked = wave.isActive !== false;
			
			// Update inactive styling
			if (wave.isActive !== false) {
				waveControl.classList.remove('inactive-control');
				waveControl.querySelectorAll('input, select, button').forEach(element => {
					if (!element.classList.contains('waveActive')) {
						element.disabled = false;
					}
				});
			} else {
				waveControl.classList.add('inactive-control');
				waveControl.querySelectorAll('input, select, button').forEach(element => {
					if (!element.classList.contains('waveActive')) {
						element.disabled = true;
					}
		...
		const typeSelect = waveControl.querySelector(`.waveType[data-index="${i}"]`);
		if (typeSelect) typeSelect.value = wave.type;
		
		const amplitudeInput = waveControl.querySelector(`.waveAmplitude[data-index="${i}"]`);
		if (amplitudeInput) amplitudeInput.value = wave.amplitude;
		
		const frequencyInput = waveControl.querySelector(`.waveFrequency[data-index="${i}"]`);
		if (frequencyInput) frequencyInput.value = wave.frequency;
		
		const phaseInput = waveControl.querySelector(`.wavePhase[data-index="${i}"]`);
		if (phaseInput) phaseInput.value = wave.phase;
		
		// Update phase value display
		const phaseValueElem = waveControl.querySelector(`.wavePhaseValue[data-index="${i}"]`);
