...
function createWaveControlsWithDirectionControl(keyframeSettings) {
	...
	// Create ALL 15 wave control rows (always create maximum)
	for (let i = 0; i < 15; i++) {
	...
	// Update which controls are visible based on waveCount
...
function initWaveDirectionControls() {
	// Initialize vector offset direction control
	initVectorOffsetDirectionControl();
 ...
 * Initialize vector offset direction control
 */
function initVectorOffsetDirectionControl() {
	const container = document.getElementById('vector-offset-direction');
	if (!container) {
		console.error('Vector offset direction container not found');
		return;
	}
	
	// Get initial angle from settings
	const activeKeyframe = appSettings.activeKeyframe;
	const initialAngle = appSettings.keyframes[activeKeyframe].offsetAngle || 0;
	
	// Create the direction control
	const control = new WaveDirectionControl(
	...
	container.__vectorOffsetControl = control;
}
