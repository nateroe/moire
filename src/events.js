...
function addResetOnDoubleClick() {
	...
	// Define the three sets of sliders
	const resetToZeroSet = new Set(['rotationOffset', 'commonHue', 'commonSaturation', 'commonLightness', 'commonDirectionOffset', 'phaseOffset', 'commonOffset']);
	const resetToOneSet = new Set(['commonThickness', 'scale', 'commonAmpFactor', 'commonFreqFactor']);
	const allResetSliders = new Set([...resetToZeroSet, ...resetToOneSet]);
	...
	document.addEventListener('mousedown', function(event) {
		...
		const isNamedSlider = allResetSliders.has(target.id);
		const isWaveSlider = target.classList.contains('wavePhase');
		const isFactorSlider = resetToOneSet.has(target.id); // controls a factor not an offset (affects text rendering)
		...
		if (lastClickTarget === target && timeSince < 300) {
			...
			if (isNamedSlider) {
				...
				if (numInput) numInput.value = resetValue.toString();
				if (valueDisplay) {
					if (isFactorSlider) {
						valueDisplay.textContent = "x " + resetValue.toFixed(2);
					} else {
						valueDisplay.textContent = formatSignedValue(resetValue).toFixed(2) + "°";
					}
				}
			} 
			...
			showToast2(`Reset ${name} to ${resetValue}`, event.clientX, event.clientY);
		}
...
function bindEvents() {
	...
	document.getElementById("gridCount").addEventListener("change", () => {
		...
		// Update grid controls for the active keyframe (show/hide existing ones)
		updateGridControlValues(appSettings.keyframes[appSettings.activeKeyframe]);
	...
	document.getElementById("commonSaturation").addEventListener("input", (e) => {
		...
		document.getElementById("commonSaturationValue").textContent = formatSignedValue(value);
	...
	document.getElementById("commonLightness").addEventListener("input", (e) => {
		...
		document.getElementById("commonLightnessValue").textContent = formatSignedValue(value);
	...
	document.getElementById("commonThickness").addEventListener("input", (e) => {
		...
		appSettings.keyframes[appSettings.activeKeyframe].commonThickness = "x " + value;
		saveSettings();
		requestRender();
	});
	...
	document.getElementById("commonThicknessNum").addEventListener("input", (e) => {
		...
		document.getElementById("commonThicknessValue").textContent = "x " + value.toFixed(2);
	...
	document.getElementById("scale").addEventListener("input", (e) => {
		...
		document.getElementById("scaleValue").textContent = "x " + value.toFixed(2);
	...
	document.getElementById("waveCount").addEventListener("input", () => {
		...
		// Update wave controls for the active keyframe (show/hide existing ones)
		updateWaveControlValues(appSettings.keyframes[appSettings.activeKeyframe]);
	...
	document.getElementById("phaseOffset").addEventListener("input", (e) => {
		...
		document.getElementById("phaseOffsetValue").textContent = formatSignedValue(value) + "°";
	...
	document.getElementById("commonOffset").addEventListener("input", (e) => {
		...
		document.getElementById("commonOffsetValue").textContent = formatSignedValue(value) + "°";
	...
	document.getElementById("commonAmpFactor").addEventListener("input", (e) => {
		...
		document.getElementById("commonAmpFactorValue").textContent = "x " + value.toFixed(2);
	...
	document.getElementById("commonFreqFactor").addEventListener("input", (e) => {
		...
		document.getElementById("commonFreqFactorValue").textContent = "x " + value.toFixed(2);
	...
	createAngleInputHandler("commonDirectionOffset", (value) => {
...
// Add to events.js - shared angle input handler
function createAngleInputHandler(inputId, updateCallback) {
    const input = document.getElementById(inputId);
    const numInput = document.getElementById(inputId + "Num");
    const valueDisplay = document.getElementById(inputId + "Value");
    
    // Slider handler
    input.addEventListener("input", (e) => {
        const value = parseFloat(e.target.value);
        if (numInput) numInput.value = value.toFixed(1);
        if (valueDisplay) valueDisplay.textContent = formatSignedValue(value) + "°";
